# ================================================================
# Script 03: Newton Inversion
# ================================================================

# This script solves the deterministic inverse problem with a full Newton method and prints the final data fit.
#
# Variable guide
# - t is the vector of sample locations / times.
# - d is the observed data vector.
# - m0 is the initial model for Newton iterations.
# - m is the current model estimate during the iteration loop.
# - mest is the final Newton estimate.
# - mref is the reference model used in Φm.
# - σd is the assumed data standard deviation.
# - ϵ2 is the regularization weight.
# - Δm is the Newton model update.
...
# - α_backtracking(...): chooses a safe Newton step length by line search.
# - solve_newton(...): runs the full Newton inversion and returns the estimate and iteration histories.
# - main(): loads the teaching case, runs Newton, and prints diagnostics.
...
#------ Let's solve the inverse problem with Newton's method ----------
# Newton uses the full Hessian, so it keeps the residual-weighted second-derivative term.
# That usually gives better local curvature information, but it is also more expensive.
...
function solve_newton(t, d, m0, σd, mref, ϵ2, maxiter, gtol)
    m = copy(m0)
    ...
    println("Stopped after $(maxiter) Newton iterations.")
    return m, Φ_history, ∇Φ_history
end

#------ Let's run the full Newton example and print the final diagnostics ----------

function main()
    (; t, d, mtrue, m0, mref, σd, ϵ2, maxiter, gtol) = generate_synthetic_geophysical_case()
    print_summary(mtrue, m0, mref, σd, ϵ2)
    mest, _, ∇Φ_history = solve_newton(t, d, m0, σd, mref, ϵ2, maxiter, gtol)
    dpred = gg(mest, t)

    println()
    print_fit_summary("Final Newton estimate", mest, t, d, σd, mref, ϵ2)
